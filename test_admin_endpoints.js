...
async function testAdminEndpoints() {
  ...
  try {
    ...
    if (allPurchasesResponse.data.length > 0) {
      ...
      const detailResponse = await axios.get(`${URLBASE}/orders/all/detail/${firstOrder.id}`, {
        headers: { Authorization: `Bearer ${testToken}` }
      });
      console.log(`✅ GET /orders/all/detail/${firstOrder.id} - Status: ${detailResponse.status}`);
      console.log(`   Productos en la orden: ${detailResponse.data.purchasesDetail.length}\n`);

      // 3. Probar actualizar estado de pedido
      console.log(`3. Probando PUT /orders/${firstOrder.id}/status`);
      const updateResponse = await axios.put(`${URLBASE}/orders/${firstOrder.id}/status`, 
        { order_status: 2 }, // Cambiar a "Retiro en Tienda"
        {
          headers: { Authorization: `Bearer ${testToken}` }
        }
      );
      console.log(`✅ PUT /orders/${firstOrder.id}/status - Status: ${updateResponse.status}`);
